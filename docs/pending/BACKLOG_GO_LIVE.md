...
| `S-02a` | RLS + policy em `users`, `clinics`, `professionals` | `[x]` | — | ✅ **Feito 2026-09-05.** Migration `0016` aplicando `ENABLE ROW LEVEL SECURITY` + `FORCE ROW LEVEL SECURITY` em `clinics`, `users`, `professionals` e `terms_acceptances`. `db/session.py` atualizado para: (1) setar `app.professional_id` e consultar/setar `app.clinic_id` em `_set_tenant()`; (2) setar `app.bypass_tenant = 'true'` em `unsafe_session_without_tenant()`; (3) resetar os três GUCs no checkin do pool. Proteção ativa no Postgres para a role `estetica_app`. |
| `S-02b` | `UserRepository`/`ClinicRepository` com escopo de clínica | `[x]` | S-02a | ✅ **Feito 2026-09-05.** `UserRepository` e `ClinicRepository` atualizados para aceitar `clinic_id: UUID | None` com filtros explícitos em `list_all()`, `get_by_id()`, `count()`, e validação contra gravação cross-tenant em `add()`. Em `deps.py`, `get_user_service` e `get_clinic_service` injetam o `clinic_id` do profissional autenticado. |
| `S-02c` | Teste de isolamento cross-tenant para `users`/`clinics`/`professionals` | `[x]` | S-02a | ✅ **Feito 2026-09-05.** Criado `tests/test_isolation_platform.py` cobrindo duas clínicas e dois tenants distintos: provado que profissional Alfa não vê clínica Beta, nem usuários Beta, nem profissionais Beta (via Repo e via query crua no Postgres com RLS), provado bloqueio de escrita cross-tenant, e provado que `checkin` do pool zera os GUCs sem vazamento. Todos os 293 testes do backend passando. |
...
| ID | Task | Status | Depende | Nota |
|---|---|:--:|---|---|
| `G-01` | Host do frontend + **rewrite de SPA** | `[x]` | B-04 | ✅ **Feito 2026-09-05.** Configurado para múltiplos provedores: `frontend/public/_redirects` (Netlify, Cloudflare Pages, Render), `frontend/vercel.json` (Vercel) e `frontend/nginx.conf` + `frontend/Dockerfile` (deploy conteinerizado em Railway, Render, etc com Nginx alpine). F5 em qualquer rota de SPA redireciona para `index.html` com status 200 |
...
| `G-03` | CI: `pytest` + `ruff` + `tsc -b` + `lint` em PR | `[x]` | G-04 | ✅ **Feito 2026-09-05.** Workflow `.github/workflows/ci.yml` criado cobrindo backend (Postgres 16 de serviço, criação de roles/grants, alembic upgrade head, ruff check e pytest -q com 286 testes) e frontend (Node 22, npm ci, tsc -b, oxlint, vitest e vite build). Removido `@oxlint/binding-win32-x64-msvc` de package.json (V1-03) para não quebrar runners Linux |
...
| `G-05` | CI: migration nova sem RLS falha o build | `[x]` | G-03, S-02a | ✅ **Feito 2026-09-05.** O teste `test_toda_tabela_com_professional_id_tem_rls_nas_migrations` em `test_architecture.py` agora roda automaticamente no CI configurado em `.github/workflows/ci.yml`. Toda nova migration com `professional_id` sem RLS/policy quebra o build no GitHub Actions |
...
| `G-07` | Backup automático **com restore testado** | `[x]` | — | ✅ **Feito 2026-09-05.** Scripts auditados criados em `scripts/backup/` (`backup.sh` com dump custom -Fc e hash SHA256, `restore.sh` com validação de checksum e `verify_backup_restore.sh`). Executada validação completa contra o Postgres: backup $\rightarrow$ criação de banco temporário $\rightarrow$ restore $\rightarrow$ auditoria comparativa de todas as 15 tabelas (100% de integridade) $\rightarrow$ validação de RLS ativo pós-restore $\rightarrow$ drop do banco de teste. Teste automatizado de integração adicionado em `tests/test_backup_restore.py` (2 passed) |
| `G-08` | Sentry (ou equivalente) no backend e frontend | `[x]` | — | ✅ **Feito 2026-09-05.** Integrado `sentry-sdk[fastapi]` no backend (inicialização segura com `send_default_pii=False`, fail-safe sem DSN) e `@sentry/react` no frontend. Conectado ao `GlobalErrorBoundary` e ao cliente de telemetria `logger.ts` (substituindo o antigo mock). Teste automatizado de integração em `test_sentry_integration.py` provando que não falha sem DSN e configura proteções LGPD quando ativo. |
...
| `G-10` | Política de Privacidade e Termos publicados + aceite registrado | `[x]` | — | ✅ **Feito 2026-09-05.** Migration `0015` criando tabela imutável `terms_acceptances` (LGPD append-only com `user_id`, `terms_version`, `ip_address`, `user_agent`, `accepted_at`) e colunas `terms_accepted_at`/`terms_version` em `users`. Novo endpoint `POST /api/v1/users/me/accept-terms` e campo `@computed_field terms_accepted` no `UserOutput`. No frontend: páginas públicas `/termos` e `/privacidade` (baseadas no `docs/LGPD_CONTRATO_OPERADOR.md`), links no login, e modal bloqueante `TermsAcceptanceModal` em `RequireAuth.tsx` impedindo uso sem aceite formal prévio. Testes de integração em `test_terms_acceptance_integration.py` |
...
| `G-12` | Import gerar oportunidades de retorno retroativas | `[x]` | — | ✅ **Feito 2026-09-05.** Migration `0014` (`return_opportunities.source`, default `'SYSTEM'`, indexado). `PatientBatchImportRequest` aceita `default_procedure_id` e `generate_return_opportunities`; `PatientService.batch_import` gera `ReturnOpportunity` (source=`'IMPORT'`, status=`OPEN`, due_date retroativo/hoje) no ato do cadastro, dando fila no dia 1. `ReturnOpportunityRepository.list_attributed()` filtra explicitamente `opp.source != 'IMPORT'`, garantindo que oportunidades importadas **nunca** inflem a receita atribuível do software (§18.1). Na UI (`PatientImportPage.tsx`), etapa de revisão permite selecionar procedimento e etapa final exibe contador de retorno com botão direto para "Quem Chamar Hoje". 2 testes novos em `test_patient_import.py` (unit + integração Postgres) |
