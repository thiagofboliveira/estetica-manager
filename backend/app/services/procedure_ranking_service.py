...
class ProcedureRankingService:
    ...
    def get_aggregated_ranking(
        ...
        *,
        ...
        all_items: list[ItemForRanking] = []
        combined_session_counts: dict[UUID, int] = {}

        for pid in professional_ids:
            with tenant_session(pid) as sess:
                si_repo = SaleItemRepository(sess, pid)
                proc_repo = ProcedureRepository(sess, pid)
                sess_repo = SessionRepository(sess, pid)

                pairs = si_repo.list_with_sale_totals_in_period(
                    period.date_from, period.date_to
                )

                procedure_names: dict[UUID, str] = {}
                for item, _sale in pairs:
                    if item.procedure_id not in procedure_names:
                        proc = proc_repo.get(item.procedure_id)
                        procedure_names[item.procedure_id] = (
                            proc.name if proc else "(procedimento removido)"
                        )
                ...
                session_counts = sess_repo.count_completed_by_procedure_in_period(
                    period.date_from, period.date_to, professional.timezone
                )
                for proc_id, cnt in session_counts.items():
                    combined_session_counts[proc_id] = (
                        combined_session_counts.get(proc_id, 0) + cnt
                    )

        ranking = build_procedure_ranking(all_items)
        ranking = [
            replace(
                row, session_count=combined_session_counts.get(row.procedure_id, 0)
            )
            for row in ranking
        ]
        return ranking, period
