    ...
    def export_sessions_csv(self) -> str:
        ...
        for sess in sessions:
            pat_name = (
                ...
                if sess.sale_item
                and hasattr(sess.sale_item, "sale")
                and sess.sale_item.sale
                and hasattr(sess.sale_item.sale, "patient")
                and sess.sale_item.sale.patient
                else ""
            )
            proc_name = (
                sess.sale_item.procedure.name
                if sess.sale_item
                and hasattr(sess.sale_item, "procedure")
                and sess.sale_item.procedure
                else ""
            )
            sched_str = (
            ...
            conf_str = (
                sess.confirmed_at.strftime("%d/%m/%Y %H:%M")
                if sess.confirmed_at
                else "Não confirmada"
            )
            cost_str = (
                f"{sess.cost_realized:.2f}".replace(".", ",")
                if sess.cost_realized is not None
                else "0,00"
            )
            ...
            writer.writerow(
                [
                    ...
                    proc_name,
                    sess.modality.value
                    if hasattr(sess.modality, "value")
                    else str(sess.modality),
                    sess.status.value
