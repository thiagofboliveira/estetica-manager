...
**Atualizado:** 2026-09-02 · **Progresso:** 67/86 (78%) · nenhuma bloqueada — motor de retorno + dashboard financeiro + ranking de procedimentos + correções P0 do front (T-017/T-024a/T-022b) implementados e validados contra Postgres real (T-045b revertido para `[ ]`: evidência anterior citava o teste errado; atribuição de 21d é T-090, fora de escopo desta branch) — 188 testes passando (`.venv/bin/pytest -q`), ruff limpo (`.venv/bin/ruff check .`)
...
| T-017 | `PATCH /sales/{id}` + `sale_audit` | `[x]` | T-015 | Nunca UPDATE na venda persistida (`FROZEN_FIELDS`) — estorna a original (`status=REFUNDED`) e cria venda de substituição via a mesma lógica de `create()`, ligadas por `sale_audit` (`app/models/sale_audit.py`, migration `0006_sale_audit.py`, RLS verificado). `GET /sales/{id}/audit` expõe o histórico de correções. **Limitação documentada:** recalcula com a config ATUAL, não a "do momento original" — não há versionamento por data (T-020a não implementado). Sessões da venda original não são tocadas. 5 testes de integração reais (`tests/test_sales_integration.py::TestCorrecaoDeVenda`), incl. verificação direta da linha gravada em `sale_audit` |
